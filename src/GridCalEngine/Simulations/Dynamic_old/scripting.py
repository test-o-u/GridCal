...
MySlackGenerator = DynamicModel()
MySlackGenerator.parse(slack_gen_data)

MySynGenerator = DynamicModel()
MySynGenerator.parse(syn_gen_data)

MyBus = DynamicModel()
MyBus.parse(bus_data)

MyLoad = DynamicModel()
MyLoad.parse(load_data)

MyBranch = DynamicModel()
MyBranch.parse(load_data)



# Add the buses and the generators and loads attached
bus1 = Bus('Bus 1', Vnom=20, _dynamic_model= MyBus, dynamic_index= 0)
grid.add_bus(bus1)

bus2 = Bus('Bus 2', Vnom=20, _dynamic_model= MyBus, dynamic_index= 1)
grid.add_bus(bus2)

gen1 = Generator('Slack Generator', vset=1.0, _dynamic_model= MySlackGenerator, dynamic_index= 0)
grid.add_generator(bus1, gen1)

gen2 = Generator('Sync Generator', vset=1.0, _dynamic_model= MySynGenerator, dynamic_index= 0)
grid.add_generator(bus1, gen2)

grid.add_load(bus2, Load('load 2', P=40, Q=20, _dynamic_model= MyLoad, dynamic_index = 0))

# add branches (Lines in this case)
grid.add_branch(Branch(bus1, bus2, 'line 1-2', r=0.05, x=0.11, b=0.02, _dynamic_model=MyBranch, dynamic_index=0))
